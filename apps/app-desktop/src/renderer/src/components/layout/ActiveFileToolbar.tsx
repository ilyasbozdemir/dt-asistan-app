...
export function ActiveFileToolbar(): React.JSX.Element | null {
  const { activeDosyaId, setActiveDosyaId, activeStarredDocs, fileName } = useWorkspaceStore()
      ...
      {!isDosyaWindowMode && (
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs text-slate-500 border-l border-slate-200 dark:border-slate-850 pl-3 select-none">
            <span className="font-bold text-[9px] uppercase text-slate-450 dark:text-slate-500 tracking-wider">Dosya:</span>
            <span className="font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[120px] lg:max-w-[200px]" title={fileName}>{fileName}</span>
            ...
            {institutionName && institutionName !== 'Kurum Adı Bulunamadı' && institutionName !== 'Kurum Bilgisi Bekleniyor...' && (
              <>
                ...
                <span className="font-bold text-[9px] uppercase text-slate-450 dark:text-slate-500 tracking-wider">Kurum:</span>
                <span className="font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[150px] lg:max-w-[280px]" title={institutionName}>{institutionName}</span>
              </>
            )}
            ...
            {eButceKodu && (
              <>
                <span className="text-slate-300 dark:text-slate-800 font-light">|</span>
                <span className="font-bold text-[9px] uppercase text-slate-450 dark:text-slate-500 tracking-wider">Kod:</span>
                <span className="font-mono font-semibold text-slate-705 dark:text-slate-295">{eButceKodu}</span>
              </>
            )}
          </div>
        </div>
              ...
              <React.Fragment key={asama.asama_sira}>
                ...
                <div className="relative inline-block">
                  ...
                  >
                    ...
                    >
                      {isCompleted ? <Check className="w-2.5 h-2.5" /> : asama.asama_sira}
                    </div>
      ...
      <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-700 pl-2">
        ...
        {adminUsername && (
          <div className="flex items-center gap-1.5 text-xs text-slate-500 border-r border-slate-200 dark:border-slate-800 pr-3 mr-1 shrink-0 select-none">
            <span className="font-bold text-[9px] uppercase text-slate-450 dark:text-slate-500 tracking-wider">Kullanıcı:</span>
            <span className="font-semibold text-slate-700 dark:text-slate-350 uppercase bg-slate-100/80 dark:bg-slate-800/80 px-2 py-0.5 rounded border border-slate-200/60 dark:border-slate-700/60 shadow-xs">{adminUsername}</span>
          </div>
        )}
