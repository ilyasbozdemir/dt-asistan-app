...
export function TeminSelector(): React.JSX.Element {
  ...
  const turColor: Record<string, string> = {
    ...
    hakedis: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300'
  }
  ...
  const getDosyaNoLabel = (d: any): string => {
    ...
    const yil =
      ...
      '2026'
    const isHakedis = isHakedisCheck(d)
    const prefix = isHakedis ? 'HAK' : 'DT'
    const no = d.temin_no || d.id
    return `${prefix}-${yil}/${no}`
  }
    ...
    <>
      <div className="relative" ref={containerRef}>
        {selectedDosya ? (
          ...
          >
            <div className={`p-1.5 rounded-xl shrink-0 ${isHakedisCheck(selectedDosya) ? 'bg-emerald-500/10 text-emerald-600' : 'bg-blue-500/10 text-blue-600'}`}>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 mt-3 w-[880px] max-w-[90vw] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-3 z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="flex items-center justify-between gap-2 p-1.5 border-b border-slate-100 dark:border-slate-800 mb-2">
              <div className="flex items-center gap-1 text-xs font-semibold">
                ...
                >
                  Tüm Dosyalar ({dosyalar.filter((d) => d.is_deleted !== 1).length})
                </button>
                <button
                ...
                >
                  ...
                  Doğrudan Temin ({dosyalar.filter((d) => d.is_deleted !== 1 && !isHakedisCheck(d)).length})
                </button>
                <button
                ...
                >
                  ...
                  Hakediş Dosyaları ({dosyalar.filter((d) => d.is_deleted !== 1 && isHakedisCheck(d)).length})
                </button>
              </div>
