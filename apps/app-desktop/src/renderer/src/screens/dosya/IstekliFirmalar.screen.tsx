...
export function IstekliFirmalar(): React.JSX.Element {
  ...
  const [activeFirmaForQuery, setActiveFirmaForQuery] = useState<any | null>(
    null,
  );
  ...
  const handleAddFirm = async (firma: any): Promise<void> => {
    ...
    if (
      associatedFirms.some((af) =>
        af.id === firma.id || af.firma_id === firma.id
      )
    ) {
  ...
  const handleRemoveFirm = async (teminFirmaId: number): Promise<void> => {
    if (
      !confirm("Bu tedarikçiyi dosyadan çıkarmak istediğinize emin misiniz?")
    ) return;
    try {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4 h-fit">
          ...
          <p className="text-[10px] text-slate-400">
            Eksik firmaları sol menüdeki <strong>İstekli Firma Yönetimi</strong>
            {" "}
            panelinden sisteme kaydedebilirsiniz.
          </p>
        </div>
          ...
          {loading
            ...
            ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-slate-400">
                ...
                <p className="text-xs">
                  Henüz bu doğrudan temin dosyası için herhangi bir istekli
                  firma eklenmemiş.
                </p>
              </div>
            )
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          <span
                            className={cn(
                              ...
                              (f.yasaklilik_durumu === "Sorgulanmadı" ||
                                !f.yasaklilik_durumu) &&
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                    İhale Kayıt No, Vergi/TCKN, Unvan ile detaylı KİK
                    sorgulaması yapın.
                  </p>
                </button>
                <button
                ...
                >
                  ...
                  <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                    Vergi No, Yasaklanan Adı ve TCKN/VKN ile e-Devlet kapısı
                    üzerinden sorgulama yapın.
                  </p>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-800 rounded-2xl space-y-3">
                <p className="text-[10px] text-slate-500 leading-normal">
                  Sorgulama sonucunu içeren ekran çıktısını PDF veya Görsel
                  olarak kaydedip dosyaya ekleyin.
                </p>
                {activeFirmaForQuery.yasaklilik_belgesi
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate">
                          {activeFirmaForQuery.yasaklilik_belgesi.split("/")
                            .pop()}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button
                          onClick={async () => {
                            if (
                              !confirm(
                                "Bu belgeyi dosyadan kaldırmak istediğinize emin misiniz?",
                              )
                            ) {
                              return;
                            }
                  ...
                  : (
                    <button
                      onClick={async () => {
                        try {
                          const openRes = await window.electron.ipcRenderer
                            .invoke("dialog:showOpenDialog");
                          if (
                            openRes.success && openRes.filePaths &&
                            openRes.filePaths.length > 0
                          ) {
                            const uploadRes = await window.electron.ipcRenderer
                              .invoke(
                                "workspace:upload-file",
                                openRes.filePaths[0],
                              );
                            if (uploadRes.success) {
                              const dbRes = await window.electron.ipcRenderer
                                .invoke(
                                  "db:run",
                              ...
                              if (dbRes.success) {
                                ...
                                alert(
                                  "Sorgulama belgesi başarıyla dosyaya kaydedildi.",
                                );
                              } else {
                                alert(
                                  "Veritabanına kaydedilirken hata: " +
                                    dbRes.error,
                                );
                              }
                            } else {
                              alert(
                                "Dosya yüklenirken hata: " + uploadRes.error,
                              );
                            }
                          }
                        } catch (err: any) {
                    ...
                    >
                      ...
                      <span className="text-[11px] font-bold">
                        Sonuç Belgesi Seçin ve Yükleyin
                      </span>
            ...
            <div className="space-y-3">
              ...
              <div className="flex bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-1 rounded-2xl">
                {["Sorgulanmadı", "Temiz", "Yasaklı"].map((status) => {
                  const isActive =
                    activeFirmaForQuery.yasaklilik_durumu === status;
                  return (
                    <button
                      key={status}
                      onClick={async () => {
