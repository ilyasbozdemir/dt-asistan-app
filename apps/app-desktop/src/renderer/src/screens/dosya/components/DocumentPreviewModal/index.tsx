/**
 * @deprecated
 * DİKKAT: Bu bileşen (DocumentPreviewModal) ve içerdiği string/HTML tabanlı eski şablon motoru
 * zamanla karmaşıklaştığı için ("balyozla sinek öldürmek" yerine) yerini Native React TSX
 * tabanlı yeni şablon motoruna bırakacaktır.
 *
 * YENİ TEMPLATE MOTORU:
 * Şablon motorumuz Native TSX kullanılarak sıfırdan yazılmıştır. Şu an public olarak
 * aşağıdaki adreste açık kaynak olarak yayınlanmıştır:
 * 👉 https://github.com/ilyasbozdemir/dt-asistan-template
 *
 * NOT: Şu anda harici bir repoda public olarak dursa da, ilerleyen aşamalarda
 * bir monorepo paketi olarak doğrudan bu workspace içerisine (`apps/template` vb. şeklinde)
 * entegre edilecektir. Gelecekteki geliştirmelerde o altyapı kullanılacaktır.
 */
            ...
            {!useSampleData && getMissingRequiredFields().length > 0 && (
              <div className="mx-3 mt-3 p-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/30 text-rose-800 dark:text-rose-400 rounded-xl text-xs flex flex-col gap-1.5 shadow-sm">
                ...
                <ul className="list-disc pl-4 space-y-0.5 text-rose-600 dark:text-rose-400">
                  {getMissingRequiredFields().map((f) => <li key={f}>{f}</li>)}
                </ul>
                <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">
                  Bu alanlar dolmadan belgenin yazdırılmasına/çıktı alınmasına
                  izin verilmez. Dilerseniz yukarıdan{" "}
                  <strong>&quot;Örnek Şablon Verisi&quot;</strong>{" "}
                  moduna geçerek taslak çıktısı alabilirsiniz.
                </p>
              </div>
            )}
