...
export function PrintDropdownButton({
  ...
  const stageSablons = useMemo(() => {
    ...
    return sablons.filter((s) => {
      ...
      if (
        categoryLower.startsWith("1-") || categoryLower.includes("baslangic")
      ) {
        return cat.includes("1-") || cat.includes("baslangic") ||
          cat.includes("başlangıç");
      }
      if (categoryLower.startsWith("2-") || categoryLower.includes("fiyat")) {
        return cat.includes("2-") || cat.includes("piyasa") ||
          cat.includes("fiyat");
      }
      if (
        categoryLower.startsWith("3-") || categoryLower.includes("siparis") ||
        categoryLower.includes("sozlesme")
      ) {
        return cat.includes("3-") || cat.includes("sipariş") ||
          cat.includes("sözleşme");
      }
      if (
        categoryLower.startsWith("4-") || categoryLower.includes("kabul") ||
        categoryLower.includes("odeme")
      ) {
        return cat.includes("4-") || cat.includes("kabul") ||
          cat.includes("ödeme");
      }
      if (
        categoryLower.startsWith("5-") || categoryLower.includes("klasor") ||
        categoryLower.includes("kapak")
      ) {
        return cat.includes("5-") || cat.includes("klasör") ||
          cat.includes("kapak");
      }
