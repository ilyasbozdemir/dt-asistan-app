...
function findCurrentStepIndex(currentRoute: string): number {
  const idx = STEPS.findIndex((s) =>
    currentRoute.includes(s.route.replace("/dosya/", ""))
  );
  return idx >= 0 ? idx : -1;
}
...
export function ProcessStepper(
  { currentRoute }: ProcessStepperProps,
): React.JSX.Element {
  const navigate = useNavigate();
  const currentIndex = findCurrentStepIndex(currentRoute);
  const prevStep = currentIndex > 0 ? STEPS[currentIndex - 1] : null;
  const nextStep = currentIndex < STEPS.length - 1
