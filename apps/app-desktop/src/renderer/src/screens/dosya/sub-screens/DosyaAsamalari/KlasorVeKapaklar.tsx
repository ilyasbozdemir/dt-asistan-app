...
export function KlasorVeKapaklar(): React.JSX.Element {
  ...
  const [checklist, setChecklist] = useState([
    ...
    { id: '4', label: 'Firma Teklif Mektupları (Kaşeli/İmzalı)', checked: false },
    { id: '5', label: 'Sözleşme / Sipariş Formu', checked: false },
    { id: '6', label: 'Muayene Kabul Tutanağı', checked: false },
    { id: '7', label: 'Taşınır İşlem Fişi (TİF)', checked: false },
    { id: '8', label: 'Ödeme Emri Belgesi (ÖEB) ve Ekleri', checked: false },
    { id: '9', label: 'Fatura Aslı', checked: false }
  ])
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 animate-in fade-in duration-300">
        ...
        {/* TOP STATUS BAR */}
        <div className={`flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border shadow-sm transition-colors ${isDosyaClosed ? 'bg-slate-100 dark:bg-slate-900 border-slate-300 dark:border-slate-700' : 'bg-white dark:bg-slate-900 border-emerald-200 dark:border-emerald-800/50'}`}>
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border ${isDosyaClosed ? 'bg-slate-200 dark:bg-slate-800 border-slate-300 dark:border-slate-700' : 'bg-emerald-100 dark:bg-emerald-900/40 border-emerald-300/40 dark:border-emerald-700/40'}`}>
              {isDosyaClosed ? (
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                {isDosyaClosed ? 'Bu dosyadaki işlemler bitmiş ve fiziksel olarak arşive kaldırılmıştır.' : 'Tüm adımlar tamamlandı. Fiziksel dosyanızı hazırlayıp kapatabilirsiniz.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 mb-5 overflow-hidden">
                <div className="bg-blue-500 h-1.5 transition-all duration-500 rounded-full" style={{ width: `${progressPercent}%` }}></div>
              </div>

              <div className="flex flex-col gap-2 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                {checklist.map((item) => (
                  <label key={item.id} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${item.checked ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                    <div className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center shrink-0 border ${item.checked ? 'bg-emerald-500 border-emerald-600 text-white' : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600'}`}>
                      {item.checked && <Check className="w-3.5 h-3.5" />}
                    </div>
                    <span className={`text-xs font-semibold ${item.checked ? 'text-emerald-800 dark:text-emerald-300 line-through opacity-70' : 'text-slate-700 dark:text-slate-300'}`}>
                      {item.label}
                    </span>
                    <input 
                      type="checkbox" 
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5 col-span-2">
                  <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Arşiv Lokasyonu / Dolap</label>
                  <input
                    type="text"
                    disabled={isDosyaClosed}
                    placeholder="Örn: Muhasebe Arşivi 2. Dolap"
                    value={arsivLokasyon}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-slate-600 dark:text-slate-400">Klasör / Sıra No</label>
                  <input
                    type="text"
                    disabled={isDosyaClosed}
              ...
              <div className="mt-4 flex justify-end">
                <button disabled={isDosyaClosed} className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-lg transition-colors border border-slate-200 dark:border-slate-700 disabled:opacity-50">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col h-full">
              <div className="flex items-center justify-between mb-5">
                ...
                {stageSablons.length > 0 && (
                  <PrintDropdownButton
                    ...
                    label={disableDocumentGuidance ? 'İşlemler' : 'Kapakları Yazdır'}
                  />
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="flex items-center gap-3 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-slate-700 dark:text-slate-300">Klasör Sırtlığı (Geniş/Dar)</h5>
                    <p className="text-[10px] text-slate-500 mt-0.5">Klasörünüzün dışı için sırtlık etiketi yazdırın.</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                  <FolderTree className="w-8 h-8 text-slate-400" />
                  <div>
                    <h5 className="text-xs font-bold text-slate-700 dark:text-slate-300">Kapak İçi İndeks (Dizi Pusulası)</h5>
                    <p className="text-[10px] text-slate-500 mt-0.5">Fiziksel dosya içindeki evrak listesi indeksini yazdırın.</p>
                  </div>
                </div>
              </div>
