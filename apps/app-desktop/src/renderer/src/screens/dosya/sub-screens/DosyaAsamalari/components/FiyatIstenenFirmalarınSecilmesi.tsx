...
function RowMenu({
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-[300] w-56 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl py-1 animate-in fade-in zoom-in-95 duration-100">
          {/* Belge işlemleri */}
          {(onFiyatPiyasaFormu || onBirimFiyatArastirmasi) && (
            <>
              <p className="px-3 pt-2 pb-1 text-[9px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                Belgeler
              </p>
              {onFiyatPiyasaFormu && (
                <button
          ...
          >
            ...
            <span className="flex-1 text-left">
              {unvanKullan ? 'Unvan kullanılıyor' : 'Sayın İlgili kullanılıyor'}
            </span>
            {/* Toggle pill */}
            <span
              className={[
