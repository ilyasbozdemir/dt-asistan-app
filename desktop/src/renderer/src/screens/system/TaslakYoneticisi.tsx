...
export default function TaslakYoneticisi(): React.JSX.Element {
  ...
  const handleToggleSelect = (docName: string): void => {
  ...
  const selectAllActiveDocs = (): void => {
  ...
  const handleAddSelectedToPreset = async (): Promise<void> => {
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-xs text-slate-500 mb-6">
                Sistemdeki tüm süreç belgeleri aşağıda listelenmiştir. Aşamalara
                tıklayarak belgeleri filtreleyebilir, checkbox ile seçtiğiniz
                belgeleri topluca pakete veya hızlı erişime ekleyebilirsiniz.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {(groupedSablonlar[activeStage] || []).map((sablon) => {
                    ...
                    const isSelected = selectedDocs.has(sablon.ad);
                    const { status, cleanName } = parseStatusAndName(
                      sablon.ad,
                      sablon.aciklama,
                    );
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1 pr-2">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => handleToggleSelect(sablon.ad)}
                          ...
                          {route ? (
                            ...
                            <div className="flex items-center gap-2 truncate text-xs font-medium text-slate-400 dark:text-slate-500 cursor-default select-none">
                              <span>{cleanName}</span>
                              {status && (
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold uppercase tracking-wide shrink-0 ${
                ...
                <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-150 dark:border-slate-850 rounded-2xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
                    >
                      Temizle
                    </button>
                    <button
                      onClick={handleAddSelectedToPreset}
                      disabled={selectedDocs.size === 0}
                      className="px-4 py-1.5 rounded-lg text-xs font-bold transition-all bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
                    >
                      Seçilenleri Pakete Ekle
                    </button>
                  </div>
                </div>
              </div>
