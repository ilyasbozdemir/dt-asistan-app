...
export function TakipScreen(): React.JSX.Element {
  ...
  useEffect(() => {
    if (activeDosya) {
      ...
      try {
        setStarredDocs(
          activeDosya.starred_docs ? JSON.parse(activeDosya.starred_docs) : [],
        );
      } catch {
        setStarredDocs([]);
      }
      try {
        setSkippedDocs(
          activeDosya.skipped_docs ? JSON.parse(activeDosya.skipped_docs) : [],
        );
      } catch {
        setSkippedDocs([]);
      }
  ...
  const toggleStar = (docName: string) => {
    ...
    if (starredDocs.includes(docName)) {
      updated = starredDocs.filter((d) => d !== docName);
  ...
  const toggleSkip = (docName: string) => {
    let updated;
    if (skippedDocs.includes(docName)) {
      updated = skippedDocs.filter((d) => d !== docName);
  ...
  const stages = dbAsamalar.length > 0 ? dbAsamalar : [
    ...
    },
    {
      ...
      aciklama:
        "Tekliflerin toplandığı ve yaklaşık maliyetin belirlendiği aşama.",
    },
    {
      asama_sira: 3,
  ...
  const getDocumentStatus = (docName: string) => {
    ...
    if (
      lowerName.includes("yaklaşık maliyet") ||
      lowerName.includes("fiyat araştırma")
    ) {
      return activeDosya.yaklasik_maliyet > 0 ? "tamamlandi" : "aktif";
    }
    if (lowerName.includes("onay")) {
      return activeDosya.durum_asama_id && activeDosya.durum_asama_id >= 3
        ? "tamamlandi"
        : "aktif";
    }
    if (
    ...
    if (
      ...
      (lowerName.includes("sözleşme") || lowerName.includes("firma"))
    ) {
      return "tamamlandi";
    }
  ...
  useEffect(() => {
    if (activeDosya && !notificationSent) {
      ...
      if (activeDosya.son_teklif_verme_tarihi) {
        const diffMs = new Date(activeDosya.son_teklif_verme_tarihi).getTime() -
          Date.now();
      ...
      if (deadlineMsg || missingCount > 0) {
        const notification = new window.Notification(
          "DT Asistan - Akıllı Hatırlatıcı",
          {
            body: `${deadlineMsg}${
              missingCount > 0
                ? `İmzası eksik ${missingCount} evrakınız bulunuyor.`
                : ""
            }`,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Doğrudan temin dosyalarınızın yasal işlem adımlarını ve belge
            tamamlama durumlarını buradan izleyebilirsiniz.
          </p>
        </div>
      </div>
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        <p className="text-xs text-slate-550 dark:text-slate-400 leading-relaxed">
                          {asama.aciklama ||
                            "Bu aşama için bir açıklama girilmemiş."}
                        </p>
                      </div>
                    </div>
                  );
                ...
                {activeAlimTuru
                  ? (
                    <div className="space-y-3">
                      <p className="text-[11px] text-slate-450 mb-2">
                        Bu alım türü için mevzuata göre dosyada bulunması
                        gereken evraklar ve dinamik durumları:
                      </p>

                      {(() => {
                        const documentList = orderedDocs ||
                          activeAlimTuru.belgeler.map((b: any) =>
                            typeof b === "string" ? b : b?.ad || ""
                          );
                        return documentList.map(
                          (documentName: string, idx: number) => {
                            const status = getDocumentStatus(documentName);
                            const isStarred = starredDocs.includes(
                              documentName,
                            );
                            const isSkipped = skippedDocs.includes(
                              documentName,
                            );
                  ...
                  : (
                    <div className="p-4 rounded-xl bg-amber-50/40 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/30 text-xs text-amber-700 dark:text-amber-400 flex items-start gap-2">
                      ...
                      <span>
                        Bu alım türü için ("{activeDosya.tur}") tanımlı bir
                        belge rehberi şablonu bulunamadı. Mevzuat ve Sistem
                        ayarlarından rehber tanımlayabilirsiniz.
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">
                        Sistemden üretilmiş dosyaların ıslak imzalı kopyalarını
            ...
            <div className="p-8 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 text-center flex flex-col items-center justify-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 max-w-md mx-auto mt-2 leading-relaxed">
                  Süreçlerin aşama aşama takibini ve evrak kontrolünü görmek
                  için listeden bir dosya seçerek aktif hale getirin veya tüm
                  listeye gidin.
                </p>
              </div>
              <Link to="/dosyalar">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-2">
                  {dosyalar.slice(0, 5).map((dosya) => {
                    ...
                    const stageName = stageInfo?.asama_adi ||
                      "Süreç Başlangıcı";
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-slate-500">
                Şu anki yıldızlı ve atlanmış belgeler konfigürasyonunuzu bir
                taslak olarak kaydederek diğer dosyalarda hızlıca
                uygulayabilirsiniz.
              </p>
              <div>
