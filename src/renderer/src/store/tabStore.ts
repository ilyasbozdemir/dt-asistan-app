...
export function getTabLabel(fullPath: string): string {
  ...
  if (path.startsWith('/dosya/hazirlik-ve-ihtiyac')) return 'Hazırlık ve İhtiyaç'
  if (path.startsWith('/dosya/piyasa-fiyat-arastirmasi')) return 'Piyasa Fiyat Araştırması'
  if (path.startsWith('/dosya/siparis-ve-sozlesme')) return 'Sipariş & Sözleşme'
  if (path.startsWith('/dosya/kabul-ve-odeme')) return 'Kabul & Ödeme İşlemleri'
  if (path.startsWith('/dosya/klasor-ve-kapaklar')) return 'Klasör & Kapaklar'
